...
//                'meta_value'   => '1',
    ...
    private function record_in_jsonTopDeals(){
        /**
         * Сначало удаляем все файлы из папки /array-page/categories/jsons
         */
        foreach (glob(get_template_directory() ."/array-page/categories/jsons/*.json") as $filename) {
            unlink($filename);
        }

        /**
         * Потом просто добавляются файлы в папку в формате json
         */
        $terms_allCategories = (object)$this->arrCategoriesPages;
        foreach($terms_allCategories as $term):
            $allCategories = get_template_directory() . '/array-page/categories/jsons/'.$term['term_slug'].'.json';
//             Запись.
