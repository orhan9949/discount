...
function home_array()
{
    ...
    $page6_slider_2 = get_field('slider_2', 6);
    ...
    foreach ($page6_slider_2 as $i => $page) {

        $slider_2['slider_2'][$i]["image_link"] = $page['image']['sizes']['large'];
        $slider_2['slider_2'][$i]["link"] = $page['link'];
        $slider_2['slider_2'][$i]["post_id"] = $page['post_id'];
        $cat_id = $wpdb->get_results('SELECT term_taxonomy_id FROM wp_term_relationships WHERE object_id = ' . url_to_postid($page['link']) . ' LIMIT 1');
        foreach ($cat_id as $ci) {
            if ($cat_id != null) {
                $slider_2['slider_2'][$i]["category_id"] = $ci->term_taxonomy_id;
            } else {
                $slider_2['slider_2'][$i]["category_id"] = [];
            }
        }
    }
    ...
    for($i = 0; $i < 1; $i++) {
        ...
        foreach ($save_wpq as $ii => $item) {
            ...
            $best_arr["best_deals"][$i]["post"][$ii]["price"] = round(get_field("price", $item->ID));
            $best_arr["best_deals"][$i]["post"][$ii]["old_price"] = round(get_field("old_price", $item->ID));
    ...
    foreach ($get_cat as $term_one) {
        if( $term_one->parent == false):
            ...
            foreach ($get_cat as $term) {
                ...
                if ($term_one->term_id == get_term($term->term_id)->parent && get_term($term->term_id)->count >= 6 && $term->name != 'Others') {
                    ...
                    foreach ($items as $ii => $item) {
                        ...
                        $posts_obj["price"] = round(get_field("price", $item->ID));
                        $posts_obj["old_price"] = round(get_field("old_price", $item->ID));
...
//    }
    ...
    array_push($bookshelf , $slider_1 , $slider_2 , $slider_3 );
    array_push($bookshelf , $best_arr, $posts_arr2, $categories_shops);
    ...
    $filename = get_template_directory() . '/array-page/home/object.json';
