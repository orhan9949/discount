...
function Filter_cat_func(WP_REST_Request $request)
{
    ...
    if ($discountFrom == false):
        $discountFrom = 1;
    endif;
    $discountTo = $request['discountTo'];
    if ($discountTo == false):
        $discountTo = 10000000000;
    endif;
    $cat_or_shop     = $request['cat_or_shop'];
    $args = array(
        'post_type' => 'products',
        'post_status' => 'publish',
        'posts_per_page' => 100,
    ...
    if($cat && $cat_or_shop){
        $cat_or_shop_Taxonomy = 'categories';
        if($cat == $cat_or_shop_Taxonomy):
            $cat_or_shop_Taxonomy = 'categories-shops';
        endif;
        $args = array_merge($args , array(
            'tax_query' => array(
                array(
    ...
    foreach ($my_posts->posts as $i => $pos) {
        ...
        if ($dopolnitelnye_foto):
            foreach ($dopolnitelnye_foto as $dop_f) {
                array_push($posts_arr[$i]->images_url, $dop_f['foto']);
            }
        endif;
...
add_action('rest_api_init', function () {
    ...
    register_rest_route('theme/v1', '/filter_cat?page=(?P<page>.+)&cat=(?P<cat>.+)&tax_slug=(?P<tax_slug>.+)&sort=(?P<sort>.+)&sortby=(?P<sortby>.+)&priceFrom=(?P<priceFrom>.+)&priceTo=(?P<priceTo>.+)&cat_or_shop=(?P<cat_or_shop>.+)&discountFrom=(?P<discountFrom>.+)&discountTo=(?P<discountTo>.+)', [
        'methods' => 'GET',
        'callback' => 'Filter_cat_func',
        'permission_callback' => '__return_true',
    ]);
