// $(document).ready(function(){
    ...
    function json_obj(cat_json,slug){
        fetch('/riza/wp-content/themes/theme/array-page/categories/page-categories.json')
        .then((response) => {
        ...
        for(let i in cat_json){
            if(slug == cat_json[i]['slug']){
                ...
                for(let ii in cat_json[i]['child']){
                    ...
                    $('.cat-detail__body-items').append('<a href="'+ url +'" target="_blank" class="cat-detail__body-item"><img src="'+ img +'" alt=""><h4>'+  title  +'</h4></div>');
                     }
            }
